                ...
                while True:
                    if os.path.exists(os.path.join(main_job_dir, 'results.csv')):
                        ...
                        if completed_jobs == num_combinations:
                            ...
                            if 'runtime_seconds' in results.columns and len(results['runtime_seconds'].dropna()) > 0:
                                avg_runtime = results['runtime_seconds'].dropna().mean()
                                st.info(f"Average runtime per job: {avg_runtime:.2f} seconds.")
                            break
                        else:
                            remaining = num_combinations - completed_jobs
                            if 'runtime_seconds' in results.columns and len(results['runtime_seconds'].dropna())>0:
                                avg_runtime = results['runtime_seconds'].dropna().mean()
                                est_remaining = (avg_runtime * remaining)/60.0
                                msg = f"{completed_jobs}/{num_combinations} done. Est remaining: {format_minutes(est_remaining)}"
                            else:
...
def monitor_job(job_id):
    ...
    if str(job_id) in result.stdout:
        print_with_time(f"Job {job_id} is still running...")
        return True
    else:
        ...
        if 'COMPLETED' in sacct_result.stdout:
            print_with_time(f"Job {job_id} completed successfully.")
            return False
        else:
            print_with_time(f"Job {job_id} did not complete successfully.")
            return False
...
def display_leaderboard(results, selected_model):
    ...
    st.write("Select hyperparameters and metrics to visualize relationships.")

    available_metrics = []
    for m in ['MSE','RMSE','R2','MAE','Accuracy']:
        if m in results.columns:
            available_metrics.append(m)

    hyperparameters = [c for c in results.columns if c not in ['Task ID','MSE','MAE','error','runtime_seconds','Accuracy','R2','RMSE']]

    if not hyperparameters:
        st.write("No hyperparameter columns found in results. Can't visualize.")
        return

    if not available_metrics:
        st.write("No metrics found in results. Can't visualize.")
        return

    x_axis = st.selectbox("Select X-axis (Hyperparameter):", hyperparameters, key='x_axis_selectbox', disabled=st.session_state["job_running"])
    y_axis = st.selectbox("Select Y-axis (Metric):", available_metrics, key='y_axis_selectbox', disabled=st.session_state["job_running"])

    for col in hyperparameters:
        if results[col].dtype == 'object':
            results[col] = results[col].astype(str)

    if y_axis not in results.columns:
        st.write(f"Selected metric {y_axis} not found in results. Please choose another metric.")
        return
    ...
    fig = px.scatter(plot_df, x=x_axis, y=y_axis, hover_data=hyperparameters + available_metrics)
    fig.update_layout(title=f"{y_axis} vs {x_axis}", xaxis_title=x_axis, yaxis_title=y_axis)
    st.plotly_chart(fig)
    print_with_time("Visualization displayed.")
